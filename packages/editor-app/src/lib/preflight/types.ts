...
//
// Preflight types and the default rule set, vendored from
// `@artworkpdf/document-model` so the published package has no
// workspace-only runtime dependency. Kept structurally identical so
// internal hosts that already import from document-model see no
